...
<svg viewBox="0 0 720 420" role="img" aria-label="Spatial query planner decision flow diagram" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- Background -->
  <rect y="0" x="0" width="720" height="420" rx="12" fill="var(--surface, #f5f3ff)"/>
  <!-- Start -->
...
Most spatial plan reading comes down to recognising five node shapes.

<svg viewBox="0 0 720 266" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Reading a spatial plan node by node: Healthy, Suspicious" style="width:100%;max-width:720px;display:block;margin:1.5rem auto;">
  <title>Reading a spatial plan node by node</title>
  <desc>A comparison table. Index Scan using …_gix: Healthy yes, Suspicious no. the predicate matched the index Bitmap Heap Scan + Recheck: Healthy yes, Suspicious partly. fine if recheck count is small Seq Scan with a spatial Filter: Healthy no, Suspicious yes. the predicate was not indexable Sort before Limit on &lt;-&gt;: Healthy no, Suspicious yes. KNN ordering lost its index Append over every partition: Healthy no, Suspicious yes. nothing pruned Two words decide most diagnoses: Index Cond means the index worked, Filter means it did not.</desc>
  <rect x="0" y="0" width="720" height="266" rx="10" fill="var(--surface, #f5f3ff)"/>
  <text x="20" y="28" font-size="12.5" font-weight="700" fill="currentColor">Reading a spatial plan node by node</text>
  <rect x="20" y="40" width="680" height="26" rx="4" fill="var(--surface-alt, #ede8f8)"/>
  <text x="286" y="58" font-size="10" font-weight="700" fill="currentColor">Healthy</text>
  <text x="394" y="58" font-size="10" font-weight="700" fill="currentColor">Suspicious</text>
  <text x="34" y="88" font-size="10.5" fill="currentColor">Index Scan using …_gix</text>
  ...
  <text x="460" y="88" font-size="9.5" fill="var(--muted, #7c6fb0)">the predicate matched the index</text>
  <line x1="20" y1="98" x2="700" y2="98" stroke="var(--viz-grid, #d8cff0)" stroke-width="1"/>
  <text x="34" y="120" font-size="10.5" fill="currentColor">Bitmap Heap Scan + Recheck</text>
  ...
  <text x="460" y="120" font-size="9.5" fill="var(--muted, #7c6fb0)">fine if recheck count is small</text>
  <line x1="20" y1="130" x2="700" y2="130" stroke="var(--viz-grid, #d8cff0)" stroke-width="1"/>
  <text x="34" y="152" font-size="10.5" fill="currentColor">Seq Scan with a spatial Filter</text>
  ...
  <text x="460" y="152" font-size="9.5" fill="var(--muted, #7c6fb0)">the predicate was not indexable</text>
  <line x1="20" y1="162" x2="700" y2="162" stroke="var(--viz-grid, #d8cff0)" stroke-width="1"/>
  <text x="34" y="184" font-size="10.5" fill="currentColor">Sort before Limit on &lt;-&gt;</text>
  ...
  <text x="460" y="184" font-size="9.5" fill="var(--muted, #7c6fb0)">KNN ordering lost its index</text>
  <line x1="20" y1="194" x2="700" y2="194" stroke="var(--viz-grid, #d8cff0)" stroke-width="1"/>
  <text x="34" y="216" font-size="10.5" fill="currentColor">Append over every partition</text>
  ...
  <text x="460" y="216" font-size="9.5" fill="var(--muted, #7c6fb0)">nothing pruned</text>
  <text x="20" y="252" font-size="10.5" fill="var(--muted, #7c6fb0)">Two words decide most diagnoses: Index Cond means the index worked, Filter means it did not.</text>
</svg>
...
Index presence is only half the story; the planner also needs statistics that describe the data it holds.

<svg viewBox="0 0 720 232" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Same query, four index states: no spatial index 2 100 ms, GiST, stale statistics 340 ms, GiST, ANALYZE run 9 ms, GiST + covering columns 6 ms — index-only for the hot path" style="width:100%;max-width:720px;display:block;margin:1.5rem auto;">
  <title>Same query, four index states</title>
  <desc>A horizontal bar chart. no spatial index is 2 100 ms. GiST, stale statistics is 340 ms. GiST, ANALYZE run is 9 ms. GiST + covering columns is 6 ms — index-only for the hot path. The jump from the second row to the third costs nothing but an ANALYZE, and it is the most commonly skipped step after a bulk load.</desc>
  <rect x="0" y="0" width="720" height="232" rx="10" fill="var(--surface, #f5f3ff)"/>
  <text x="20" y="28" font-size="12.5" font-weight="700" fill="currentColor">Same query, four index states</text>
  <text x="20" y="61" font-size="10.5" fill="currentColor">no spatial index</text>
  <rect x="250" y="48" width="340" height="18" rx="3" fill="var(--viz-bad, #a32b23)" opacity="0.75"/>
  <text x="598" y="61" font-size="10" font-weight="700" fill="var(--viz-bad, #a32b23)">2 100 ms</text>
  <text x="20" y="95" font-size="10.5" fill="currentColor">GiST, stale statistics</text>
  <rect x="250" y="82" width="55" height="18" rx="3" fill="var(--viz-warn, #8a5000)" opacity="0.75"/>
  <text x="313" y="95" font-size="10" font-weight="700" fill="var(--viz-warn, #8a5000)">340 ms</text>
  <text x="20" y="129" font-size="10.5" fill="currentColor">GiST, ANALYZE run</text>
  ...
  <text x="20" y="163" font-size="10.5" fill="currentColor">GiST + covering columns</text>
  <rect x="250" y="150" width="6" height="18" rx="3" fill="var(--viz-good, #1f6b3a)" opacity="0.75"/>
  <text x="264" y="163" font-size="10" font-weight="700" fill="var(--viz-good, #1f6b3a)">6 ms — index-only for the hot path</text>
  <text x="20" y="200" font-size="10.5" fill="var(--muted, #7c6fb0)">The jump from the second row to the third costs nothing but an ANALYZE, and it is the most commonly skipped step after a bulk load.</text>
</svg>

One discipline underpins everything in this section: capture the plan before changing anything, and capture it again afterwards. A tuning change without a before-and-after pair is an opinion, and spatial plans are counter-intuitive often enough that opinions are frequently wrong.
